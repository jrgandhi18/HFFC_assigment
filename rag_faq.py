...
def fetch_homefirst_faqs(limit: int = 30) -> list[FAQDoc]:
    ...
    return [
        ...
        FAQDoc("What is FOIR in home loan?", "FOIR is the share of your monthly income used for total loan repayments. Lower FOIR generally improves approval chances."),
        FAQDoc("What is LTV in home loan?", "LTV is Loan-to-Value ratio: loan amount divided by property value. Lower LTV usually lowers risk for lender."),
        FAQDoc("How much down payment is usually needed?", "If LTV is 80%, the remaining 20% is typically paid by the customer as down payment plus other charges."),
        FAQDoc("Can self-employed applicants get home loan?", "Yes, self-employed applicants are commonly eligible with valid business and income proof."),
        FAQDoc("Can salaried applicants get better eligibility?", "Salaried applicants often have simpler income verification, but final eligibility depends on full profile."),
        FAQDoc("What is the minimum salary needed for home loan?", "There is no single fixed number in this prototype. Eligibility depends on income, obligations, LTV, and FOIR."),
        FAQDoc("Do existing loans affect home loan eligibility?", "Yes, existing EMIs increase repayment burden and can reduce eligible home loan amount."),
        FAQDoc("Can I increase tenure to reduce EMI?", "Yes, longer tenure usually reduces monthly EMI but increases total interest paid over time."),
        FAQDoc("Can I choose fixed or floating interest rate?", "Many lenders provide fixed, floating, or hybrid options. Availability depends on product and policy."),
        FAQDoc("What is sanction letter?", "A sanction letter is a conditional approval document listing loan amount, tenure, interest terms, and conditions."),
        FAQDoc("How long does home loan approval take?", "Approval time depends on profile completeness, verification, and legal/technical checks on property."),
        FAQDoc("What are legal and technical checks?", "Lenders usually verify property ownership documents and physical/market assessment before disbursal."),
        FAQDoc("Can I apply for under-construction property?", "Yes, many lenders support under-construction properties, subject to approved projects and policy checks."),
        FAQDoc("Can I transfer my existing home loan from another lender?", "Balance transfer is commonly available, subject to repayment track record and new lender eligibility."),
        FAQDoc("Is there any processing fee?", "Processing fee may apply based on lender policy and product. Exact amount can vary by case."),
        FAQDoc("Are there foreclosure charges?", "Foreclosure or pre-closure charges depend on loan type and current lender policy."),
        FAQDoc("Can I make part-payment on home loan?", "Part-payment is usually allowed. It can reduce future EMI or loan tenure based on your choice."),
        FAQDoc("Can women applicants get any benefits?", "Some products may offer benefits for women applicants or co-owners, depending on policy."),
        FAQDoc("Can I include rental income for eligibility?", "In some cases, rental income may be considered with valid proof and policy limits."),
        FAQDoc("What is the maximum age for home loan tenure?", "Age limits vary by lender policy. Final tenure usually depends on applicant age at maturity."),
        FAQDoc("Can NRI customers apply for home loan?", "Many lenders offer NRI home loans with specific documentation and compliance requirements."),
        FAQDoc("What happens if my application is rejected?", "You can review rejection reasons, improve profile factors, and reapply with better documentation or structure."),
        FAQDoc("Can I take top-up loan on existing home loan?", "Top-up loans may be available on eligible existing loans with acceptable repayment history."),
        FAQDoc("How is EMI calculated in simple terms?", "EMI depends on principal amount, annual interest rate, and tenure. Higher amount or rate increases EMI."),
        FAQDoc("What details should I share for quick eligibility check?", "Share monthly income, property value, requested loan amount, and employment status for a fast estimate."),
    ]
